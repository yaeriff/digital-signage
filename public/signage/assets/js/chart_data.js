  ...
  const gaugeScalePlugin = {
    ...
    afterDatasetsDraw: (chart, args, options) => {
      ...
      for (let i = 0; i <= 100; i += 2) {
        ...
        const startX = xCenter + Math.cos(angle) * (outerRadius + tickGap);
        const startY = yCenter + Math.sin(angle) * (outerRadius + tickGap);
        const endX =
          xCenter + Math.cos(angle) * (outerRadius + tickGap + length);
        const endY =
          yCenter + Math.sin(angle) * (outerRadius + tickGap + length);
        ...
        if (isMajor) {
          ...
          const labelRadius = outerRadius + tickGap + length + textPadding;
